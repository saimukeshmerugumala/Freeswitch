...
static switch_status_t do_config(void)
{
	...
	if ((settings = switch_xml_child(cfg, "settings"))) {
		for (param = switch_xml_child(settings, "param"); param; param = param->next) {
			...
			} else if(!strcasecmp(var, "aws-signed-url")) {
                globals.aws_signed_url = switch_true(val);
            }
		}
	}
...
switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "### in Standard App\n");
	if (!zstr(data)) {
...
switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "#########Found  profile %s \n", profile_name);
		} else {
...
static char *load_cache_data(http_file_context_t *context, const char *url)
{
    ...
    char *dup_url = NULL;
    char *splitUrl[3] = { 0 };
    // Srinivas: Checking the URLs are AWS or not
    if(globals.aws_signed_url) {
         dup_url = strdup(url);
         switch_split((char *)dup_url, '?', splitUrl );
         switch_md5_string(digest, (void *) dup_url, strlen(dup_url));
    } else {
...
static switch_status_t fetch_cache_data(http_file_context_t *context, const char *url, switch_event_t **headers, const char *save_path)
{
	...
	int tries = 10;
    //Srinivas : AWS signed URLs are different for http GET and HEAD. HT-TAPI module first try with http HEAD to get file info and 
    // then try with http GET to download the file. Because AWS signed URLs are different we can only make http GET request.
    //HT-TAPI program sets to http GET if 'save_path' is not null.
    if(globals.aws_signed_url) {
       save_path = context->cache_file;
    }  
    ...
    // Srinivas -- Setting the http method as GET so that awazon S3 signed URL can be processed. Managing it with XML paramter 'aws_signed_url'
    // switch_curl_easy_setopt(curl_handle, CURLOPT_HTTPGET,1L );
	switch_curl_easy_perform(curl_handle);
    ...
    switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO,"http response code from http URL like AWS S3: %ld", code);
	switch(code) {
	case 200:
		if (save_path) {
...
static switch_status_t locate_url_file(http_file_context_t *context, const char *url)
{
	...
	if (context->expires > 1 && now < context->expires) {
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "URL: [%s] is loaded from cache!\n", url);
		return SWITCH_STATUS_SUCCESS;
	}
	if (context->url_params) {
		ext = switch_event_get_header(context->url_params, "ext");
    ...
    switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "URL: [%s] is getting downloaded from server!\n", url);
if(!globals.aws_signed_url) {
