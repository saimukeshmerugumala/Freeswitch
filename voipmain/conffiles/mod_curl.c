...
/* Srinivas  adding reqheaders as parameter to definition*/
static http_data_t *do_lookup_url(switch_memory_pool_t *pool, const char *url, const char *method, const char *data, const char *content_type, curl_options_t *options, const char *req_headers)
{
	...
	struct data_stream dstream = { NULL };
        int reqHeaderCount, count = 0;
        char *reqHeaders[10] = { 0 };
	...
	if (!method) {
        ...
        /* Srinivas printing req headers received*/
	//switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "method: %s, url: %s, content-type: %s, req-headers: %s\n", method, url, content_type, req_headers);
	...
	if (!strcasecmp(method, "head")) {
       ...
       /* Srinivas setting request headers to curl*/
        if(req_headers){
               reqHeaderCount = switch_split((char *)req_headers, ';', reqHeaders);
               for(count = 0; count < reqHeaderCount; count++ ){
                   //switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "req header: %s\n", reqHeaders[count]);
                   headers = switch_curl_slist_append(headers, reqHeaders[count]);
                  //switch_safe_free(reqHeaders[count]);
                   reqHeaders[count] = NULL;
               }
                   
                switch_curl_easy_setopt(curl_handle, CURLOPT_HTTPHEADER, headers);
                /* switch_safe_free(reqHeaders);
                for(count = 0; count < reqHeaderCount; count++) {
                     reqHeaders[count] = NULL;
                }
                 */
...
static void http_sendfile_initialize_curl(http_sendfile_data_t *http_data)
{
	...
	curl_easy_setopt(http_data->curl_handle, CURLOPT_HTTPPOST, http_data->formpost);
       ...
       /* Srinivas set req headers to curl */
        if(!zstr(http_data->reqheaders)){
               reqHeaderCount = switch_split((char *)http_data->reqheaders, ';', reqHeaders);
               for(hcount = 0; hcount < reqHeaderCount; hcount++ ){
                   switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "curl_senfile req header: %s\n", reqHeaders[hcount]);
                   http_data->headers = switch_curl_slist_append(http_data->headers, reqHeaders[hcount]);
                   reqHeaders[hcount] = NULL;
               }
           curl_easy_setopt(http_data->curl_handle, CURLOPT_HTTPHEADER, http_data->headers);
        }
...
SWITCH_STANDARD_APP(http_sendfile_app_function)
{
	...
	switch_channel_t *channel = switch_core_session_get_channel(session);
        //int reqHeaderCount, count = 0;  /* Srinivas */
        //char *reqHeaders[10] = { 0 };	
        //switch_stream_handle_t stream = { 0 };  // Srinivas Adding stream to send curl_response	
	assert(channel != NULL);
	...
	else
	{
		...
		http_data->extrapost_elements = (char *) switch_channel_get_variable_dup(channel, "curl_sendfile_extrapost", SWITCH_TRUE, -1);
                /* Srinivas reading header parameters */ 
                http_data->reqheaders = (char *) switch_channel_get_variable_dup(channel, "curl_sendfile_reqheaders", SWITCH_TRUE, -1);
                //switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "Header parameters %s specified for curl_sendfile.\n", http_data->reqheaders);

        /*if(http_data->reqheaders){
               reqHeaderCount = switch_split((char *)http_data->reqheaders, ';', reqHeaders);
               for(count = 0; count < reqHeaderCount; count++ ){
                   switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "curl_senfile req header: %s\n", reqHeaders[count]);
                   http_data->headers = switch_curl_slist_append(http_data->headers, reqHeaders[count]);                  
                   reqHeaders[count] = NULL;
               }
        }*/		
	...
	switch_url_decode(http_data->filename_element);
        ...
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "identifier uuid %s specified for curl_sendfile.\n", http_data->identifier_str);	
...
// Srinivas adding curl_response_code so that calling program can take some action
switch_channel_set_variable(channel, "curl_sendfile_response_code", switch_core_sprintf(pool, "%ld", http_data->http_response_code));
switch_channel_set_variable(channel, "curl_senfile_response_data", http_data->sendfile_response);
//switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "curl response sendfile: %s", http_data->sendfile_response);
//switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "curl response http: %s", http_data->http_response);
...
#define HTTP_SENDFILE_SYNTAX "<url> <filenameParamName=filepath> [nopost|postparam1=foo&postparam2=bar... [event|stream|both|none  [identifier [<header1:val1;header2:val2>]]]]"
		...
		if(argc > 2)
		{
			...
			if(argc > 3)
			{
				...
				if(argc > 4)
                                {
					http_data->identifier_str = switch_core_strdup(pool, argv[i++]);
                                        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO,"curl_sendfile_uuid:%s", http_data->identifier_str);
                                }
                               if(argc > 5) 
                               {
                                   http_data->reqheaders = switch_core_strdup(pool, argv[i++]);
                                   switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO,"curl_sendfile_header:%s",http_data->reqheaders);     
                               }
			}
		}
	}
...
SWITCH_STANDARD_APP(curl_app_function)
{
	...
	char *content_type = NULL;
        char *req_headers  = NULL; // Srinivas header parameters variable
	...
	if ((argc = switch_separate_string(mydata, ' ', argv, (sizeof(argv) / sizeof(argv[0]))))) {
		...
		for (i = 1; i < argc; i++) {
			...
			} else if(!strcasecmp("req-headers", argv[i])) {    /*Srinivas reading header parameters to local varaible*/
                                if (++i < argc) {
                                        req_headers = switch_core_strdup(pool, argv[i]);
                                }
                        } 
		}
	}
	...
	if (curl_timeout) options.timeout = atoi(curl_timeout);
        /*Srinivas  checking curl timeout value. It should be set to session like session:setVariable("curl_timeout", "20"); */
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "method: %s, url: %s, curl_timeout: %s\n", method, url, curl_timeout);
	http_data = do_lookup_url(pool, url, method, postdata, content_type, &options, req_headers); // Srinivas passing headers
	if (do_json) {
...
SWITCH_STANDARD_API(curl_function)
{
	...
	char *postdata = NULL;
	char *content_type = NULL;
        char *req_headers  = NULL;  /*Srinivas */
	switch_bool_t do_headers = SWITCH_FALSE;
	switch_bool_t do_json = SWITCH_FALSE;
	switch_curl_slist_t *slist = NULL;
	http_data_t *http_data = NULL;
	...
	if ((argc = switch_separate_string(mydata, ' ', argv, (sizeof(argv) / sizeof(argv[0]))))) {
		...
		for (i = 1; i < argc; i++) {
			...
			} else if(!strcasecmp("req-headers", argv[i])) {   /* Srinivas reading header parameters to a local variable*/
                                if (++i < argc) {
                                        req_headers = switch_core_strdup(pool, argv[i]);
                                }
                        }
        ...
        /*Srinivas */
        switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "method: %s, url: %s, timeout: %ld\n", method, url, options.connect_timeout);
	http_data = do_lookup_url(pool, url, method, postdata, content_type, &options, req_headers);
		if (do_json) {
